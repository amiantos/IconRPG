...
class BattleViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        let buttons: [RPGButton] = [self.weaponButton,
                                    self.bandageButton,
                                    self.guardButton,
                                    self.magicButton,
        ...
        enemyHealthBarInterior.frame = CGRect(x: 0, y: 0,
                                              width: enemyHealthBar.frame.width,
                                              height: enemyHealthBar.frame.height)
        enemyHealthBarInterior.layer.cornerRadius = enemyHealthBarInterior.frame.height / 2
        enemyHealthBarInterior.clipsToBounds = true

        playerHealthBarInterior.frame = CGRect(x: 0, y: 0,
                                               width: playerHealthBar.frame.width,
                                               height: playerHealthBar.frame.height)
    ...
    func attack() {
        ...
        var newWidth = enemyHealthBarInterior.frame.size.width - 50
        ...
        UIView.animate(withDuration: 0.2, animations: {
            self.enemyHealthBarInterior.frame = CGRect(x: ((self.enemyHealthBar.frame.width - newWidth) / 2),
                                                       y: 0,
                                                       width: newWidth,
                                                       height: self.enemyHealthBarInterior.frame.height)
        })
    }
